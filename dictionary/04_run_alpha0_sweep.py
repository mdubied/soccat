...
AND both z-thresholds
=============================================================================
Robustness check on two free parameters: eq. (23)'s alpha0 ("prior sample
size") and the z-threshold used to select dictionary words (see
02_build_dictionary.py's module docstring for what low/medium/high and
z-threshold=0.0/1.96 mean). Runs 02_build_dictionary.py and
03_evaluate_dictionary.py once per (alpha0-mode, z-threshold) combination (6
total), then assembles a single side-by-side comparison table.
...
def run(script: str, mode: str, z: float):
    cmd = [sys.executable, str(ROOT / script), "--alpha0-mode", mode, "--z-threshold", str(z)]
    print(f"$ {' '.join(cmd)}")
...
def main():
    for mode in MODES:
        for z in Z_THRESHOLDS:
            run("02_build_dictionary.py", mode, z)
            run("03_evaluate_dictionary.py", mode, z)

    rows = []
    for mode in MODES:
        for z in Z_THRESHOLDS:
            variant = f"{mode}_{z_suffix(z)}"
            out_dir = ROOT / "output" / variant
            with open(ROOT / "dictionaries" / variant / "alpha0.json", encoding="utf-8") as f:
